...
RECEIVER_EMAIL = "bhawanagupta002@gmail.com"
...
def run_refresh_scraper(db_path=None):
    try:
        if db_path:
            global DB_PATH
            DB_PATH = db_path
        ...
        main()
        return "✅ Refresh scraper executed successfully."
    except Exception as e:
        print("❌ Error running refresh scraper:", e)
        return f"❌ Error running refresh scraper: {e}"

def run_scraper_in_background(db_path=None):
    thread = threading.Thread(target=run_refresh_scraper, kwargs={"db_path": db_path})
    thread.start()
    print("🚀 Scraper started in background thread.")
